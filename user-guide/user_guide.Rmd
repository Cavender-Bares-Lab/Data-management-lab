...
Created, remove, and update documentation is essential for any organization. 
...
This webpage does not not replaceable data storage requirements (e.g., group/cavender, 
...
If you need to create, remove, or update documentation please follow the bellows 
guide, or ask to the lab-technician for help.
...
If you and someone has been working on this webpage or you recent started it is 
crucial to have the updated version. 

If you recent started, you can clone the github repository to your local machine
...
In non-technical words, this webpage is composted by two elements: the structure and
the documentation.
...
- The documentation refers to the `.md` files located in the docs folder. These files
  ...
  a good practice to keep these files in folders under docs with names relatives 
  to tabs and sub-tabs, just to keep the webpage in order.

Now that we know this elements, let's take a look at three functions of `MkDocs` 
that we will need to use. In RStudio you can run this command using the 'Terminal'
(the tab next to 'Console' that we always use).

- `mkdocs serve`: this command help us to create local address where we can see the
